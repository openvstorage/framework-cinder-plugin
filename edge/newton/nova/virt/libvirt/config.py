...
class LibvirtConfigOpenvStorageEdgeGuestDisk(LibvirtConfigGuestDisk):
    ...
    def format_dom(self):
        ...
        etree.SubElement(source, "host", name=self.source_host_name, port=self.source_host_port, transport=self.source_host_transport)
        if self.source_name is not None:
            source.set('name', self.source_name)
        dev.append(source)
        dev.append(etree.Element("target",
                                 dev=self.target_dev,
                                 bus=self.target_bus))
        LOG.debug('logging openvstorage config 2')
        return dev
    ...
    def parse_dom(self, xmldoc):
        ...
        for c in xmldoc.getchildren():
            LOG.debug('logging openvstorage parse_dom: {0}'.format(c))
            if c.tag == 'driver':
                self.driver_name = c.get('name')
                self.driver_format = c.get('type')
            elif c.tag == 'source':
                ...
                LOG.debug('logging openvstorage parse_dom, source: {0}'.format(len(c.getchildren())))
                for sub in c.getchildren():
                    if sub.tag == 'host':
