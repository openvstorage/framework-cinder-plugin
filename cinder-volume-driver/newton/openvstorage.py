...
OPTS = [cfg.StrOpt('vpool_name',
                   default = '',
                   help = 'Vpool to use for volumes - '
                          'backend is defined by vpool not by us.')]
    ...
    for item in sorted(dir(volume)):
        if not item.startswith('__'):
            try:
                ...
                LOG.debug('DEBUG failed %s', ex)
    LOG.debug('[%s] %s', call, vol_info)
...
class OVSVolumeDriver(driver.VolumeDriver):
    ...
    def __init__(self, *args, **kwargs):
        ...
        LOG.debug('INIT %s %s %s ', CONF.vpool_name, args, kwargs)
    ...
    def copy_image_to_volume(self, context, volume, image_service, image_id):
        ...
        if destination_path:
            ...
            self._ensure_volume_is_gone(volume.provider_location,
            ...
            ovs_disk = self._find_ovs_model_disk_by_location(
                volume.provider_location, six.text_type(volume.host))
            ovs_disk.name = name
            ovs_disk.save()
    ...
    def create_cloned_volume(self, volume, src_vref):
        ...
        mountpoint = self._get_hostname_mountpoint(volume.host)
        name = volume.display_name
        if not name:
            name = volume.name
            volume.display_name = volume.name
        ...
        source_ovs_disk = self._find_ovs_model_disk_by_location(
            src_vref.provider_location, src_vref.host)
        if source_ovs_disk.info['object_type'] == 'TEMPLATE':
            ...
            available_snapshots = [snapshot for snapshot in
                                   source_ovs_disk.snapshots
                                   if 'in_backend' not in snapshot or
                                   snapshot['in_backend'] is True]
            if len(available_snapshots) == 0:
                ...
                LOG.debug('CREATE_SNAP %s %s', name, metadata)
            ...
            LOG.debug('[CLONE FROM SNAP] Meta: %s', disk_meta)
    ...
    def get_volume_stats(self, refresh=False):
        ...
        # Due to the way the backend is implemented, at the moment
        # we don't have a way of knowing the exact amount of available capacity
        # It involves taking into account the write cache, deduping and
        # erasure coding
    ...
    def create_snapshot(self, snapshot):
        ...
        vdisklib.VDiskController.create_snapshot.apply_async(kwargs={"diskguid": ovs_disk.guid,
                                                                     "metadata": metadata,
                                                                     "snapshotid": six.text_type(snapshot.id)}).get(timeout=1800)
        LOG.debug('CREATE_SNAP OK')
    ...
    def delete_snapshot(self, snapshot):
        ...
        if self._snapshot_has_children(snapshot.id):
            LOG.exception(_LE('Snapshot %s has children'), snapshot.id)
    ...
    def create_volume_from_snapshot(self, volume, snapshot):
        ...
        try:
            ...
            LOG.error(_LE('CLONE FROM SNAP: Internal error %s '), ex)
    ...
    def _snapshot_has_children(self, snapshotid):
        ...
        if hasattr(vdisklist.VDiskList, 'get_by_parentsnapshot'):
            return len(vdisklist.VDiskList.
                       get_by_parentsnapshot(snapshotid)) > 0
        else:  # on 2.0.0
    ...
    def _ensure_volume_is_gone(self, location, hostname, retry=3,
                               timeout=3):
        """Make sure OVS disk object (based on location and hostname)
    ...
    def _wait_for_snapshot(disk, snapshot_id=None, tries=60):
        """Waits for a given snapshot (or any) to be available in the backend

        (so it can be used for clones)
        """
        def load():
            disk.invalidate_dynamics(['snapshots'])
            return [snapshot for snapshot in disk.snapshots
                    if ('in_backend' not in snapshot
                        or snapshot['in_backend'] is True)
                    and (snapshot_id is None or
                         snapshot['guid'] == snapshot_id)]

        snapshots = load()
        while len(snapshots) == 0 and tries > 0:
            if snapshot_id is None:
                LOG.debug('Waiting for a snapshot to become available')
            else:
                LOG.debug('Waiting for snapshot {0} to become available'
                          .format(snapshot_id))
            tries -= 1
            time.sleep(60 - tries)
            snapshots = load()
        if tries == 0 and len(snapshots) == 0:
            ...
            raise RuntimeError('Snapshot {0} did not show up'
                               .format(snapshot_id))
