...
class OVSClient(object):
    ...
    def __init__(self, ip, username, password, verify=False, version='*', port=None, raw_response=False):
        ...
        if username is None and password is None:
            raise RuntimeError('Credentials should be None (no authentication) or a tuple containing username and client_secret (authenticated)')
        self.ip = ip
        self.port = port or 443
        self.username = username
        self.password = password
        self._url = 'https://{0}:{1}/api'.format(self.ip, self.port)
        self._key = hashlib.sha256('{0}{1}{2}{3}'.format(self.ip, self.port, self.username, self.password)).hexdigest()
        self._token = None
        self._verify = verify
    ...
    def _prepare(self, **kwargs):
        ...
        * Preparing headers, returning them
        """
        if self.username is not None and self._token is None:
            self._connect()
